...
@interface DemandDetailContactCell ()
...
@property (strong, nonatomic) UIView *lineH;/** 竖直的分割线 */

@end

@implementation DemandDetailContactCell
...
- (void)createUI
{
    ...
    self.lineH = [[UIView alloc] init];
    self.lineH.backgroundColor = PGCTintColor;
    [self.contentView addSubview:self.lineH];
    self.lineH.sd_layout
    .topSpaceToView(self.contentView, 20)
    .rightSpaceToView(self.callBtn, 20)
    .heightIs(40)
    .widthIs(1.5);
    ...
    .centerYEqualToView(self.lineH)
    .leftSpaceToView(self.contentView, 15)
    .rightSpaceToView(self.lineH, 20)
    .heightIs(1);
    ...
    .rightSpaceToView(self.lineH, 30)
    .autoHeightRatio(0);
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(demandDetailContactCell:phone:)]) {
        [self.delegate demandDetailContactCell:self phone:sender];
    }
}
...
- (void)setContact:(Contacts *)contact
{
    ...
    [self setupAutoHeightWithBottomView:self.lineH bottomMargin:20];
}
