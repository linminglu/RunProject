...
- (BOOL)supplyTopEdited
{
    if (!(self.supplyDetail.title.length > 0)) {
        [MBProgressHUD showError:@"请填写标题" toView:self.view];
        return false;
    }
    if (!(self.supplyDetail.company.length > 0)) {
        [MBProgressHUD showError:@"请填写供应商家" toView:self.view];
        return false;
    }
    if (!(self.supplyDetail.address.length > 0)) {
        [MBProgressHUD showError:@"请填写地址" toView:self.view];
        return false;
    }
    if (!(self.supplyDetail.city_id > 0)) {
        [MBProgressHUD showError:@"请选择地区" toView:self.view];
        return false;
    }
    if (!(self.supplyDetail.types.count > 0)) {
...
- (void)uploadPhotoes:(void(^)(void))complete
{
    ...
    for (int i = 0; i < self.publishImages.count; i++) {
        ...
        Images *photoModel = self.supplyDetail.images[i];
...
- (NSString *)jsonStrFromModel
{
    ...
    if (self.supplyDetail.id > 0) {
        [dic setObject:@(self.supplyDetail.id) forKey:@"id"];
    }
    [dic setObject:self.supplyDetail.title forKey:@"title"];
    [dic setObject:self.supplyDetail.company forKey:@"company"];
    [dic setObject:self.supplyDetail.address forKey:@"address"];
    [dic setObject:@(self.supplyDetail.city_id) forKey:@"city_id"];
    [dic setObject:self.supplyDetail.desc forKey:@"description"];
    
    NSMutableArray *types = [NSMutableArray array];
    for (Types *type in self.supplyDetail.types) {
        [types addObject:@(type.id)];
    }
    [dic setObject:types forKey:@"type_ids"];
    
    NSMutableArray *contacts = [NSMutableArray array];
    for (Contacts *contact in self.supplyDetail.contacts) {
...
{
    if (![self supplyTopEdited]) {
        return;
    }
    if (![self editedContacts:self.supplyDetail.contacts]) {
        return;
    }
    if (!(self.supplyDetail.desc.length > 0)) {
    ...
    [self uploadPhotoes:^{
        ...
        if (![self editedImages:self.supplyDetail.images]) {
            return;
        }
...
- (void)footerButtonClick:(UIButton *)sender
{
    if (sender.tag == 1) {
        ...
        [self.supplyDetail.contacts addObject:contact];
    }
    if (sender.tag == 3) {
        ...
        [self.supplyDetail.images addObject:image];
...
- (void)introduceSupplyTopCell:(IntroduceSupplyTopCell *)topView selectArea:(UIButton *)area
{
    ...
    areaVC.areaBlock = ^(PGCProvince *province, PGCCity *city) {
        ...
        strongSlef.supplyDetail.province_id = province.id;
        strongSlef.supplyDetail.province = province.province;
        strongSlef.supplyDetail.city_id = city.id;
        strongSlef.supplyDetail.city = city.city;
    };
...
- (void)introduceSupplyTopCell:(IntroduceSupplyTopCell *)topView selectDemand:(UIButton *)demand
{
    ...
    __weak typeof(self) weakSelf = self;
    supplyVC.typeBlock = ^(NSMutableArray<PGCMaterialServiceTypes *> *types) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        
        NSMutableArray *array = [NSMutableArray array];
        strongSelf.supplyDetail.types = [NSMutableArray array];
        for (PGCMaterialServiceTypes *model in types) {
            Types *type = [[Types alloc] init];
            type.id = model.id;
            type.parent_id = model.parent_id;
            type.name = model.name;
            type.sequence = model.sequence;
            [strongSelf.supplyDetail.types addObject:type];
            
            [array addObject:model.name];
        }
        NSString *title = [array componentsJoinedByString:@","];
        [demand setTitle:title forState:UIControlStateNormal];
    };
...
- (void)introduceDemandContactCell:(IntroduceDemandContactCell *)cell deleteBtn:(UIButton *)deleteBtn
{
    ...
    if (self.supplyDetail.contacts.count > 1) {
        [self.supplyDetail.contacts removeObjectAtIndex:indexPath.row];
    }
    [self.tableView reloadData];
}
...
- (void)introduceDemandImagesCell:(IntroduceDemandImagesCell *)cell imageView:(UIImageView *)imageView
{
    ...
    Images *model = [self.supplyDetail.images objectAtIndex:indexPath.row];
    model.isPublish = false;
    [self.supplyDetail.images replaceObjectAtIndex:indexPath.row withObject:model];
...
- (void)introduceDemandImagesCell:(IntroduceDemandImagesCell *)cell deleteBtn:(UIButton *)deleteBtn
{
    ...
    if (self.supplyDetail.images.count > 1) {
        [self.supplyDetail.images removeObjectAtIndex:indexPath.row];
...
- (UIButton *)introduceBtn {
    if (!_introduceBtn) {
        ...
        NSString *title = self.supplyDetail ? @"重新发布" : @"发布";
...
- (NSMutableArray *)dataSource {
    if (!_dataSource) {
        ...
        if (!self.supplyDetail) {
            // 新建一个空白的供应对象
            PGCSupply *supplyDetail = [[PGCSupply alloc] init];
            ...
            supplyDetail.contacts = contacts;
            ...
            supplyDetail.images = images;
            
            [_dataSource insertObject:@[supplyDetail] atIndex:0];
            [_dataSource insertObject:supplyDetail.contacts atIndex:1];
            [_dataSource insertObject:@[supplyDetail] atIndex:2];
            [_dataSource insertObject:supplyDetail.images atIndex:3];
            [self.publishImages addObject:[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"加照片"]]];
            ...
            [_dataSource insertObject:self.supplyDetail.contacts atIndex:1];
            [_dataSource insertObject:@[self.supplyDetail] atIndex:2];
            for (Images *image in self.supplyDetail.images) {
